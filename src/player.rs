...
impl Player {
    ...
    pub fn collision(&self, x:f32, y:f32) -> bool {
        let first_rect = |x, y| {
            ...
            let xcol = self.pos.x - x <= 15.0 && x - self.pos.x <=14.0;
            let ycol = self.pos.y - y <= 3.0 && y - self.pos.y <= 17.0;
            xcol && ycol
        };
        let second_rect = |x,y| {
            // second rect: 8:21, 6:34
            let xcol = self.pos.x - x <= 7.0 && x - self.pos.x <=6.0;
            let ycol = self.pos.y - y <= 11.0 && y - self.pos.y <= 17.0;
            xcol && ycol
        };
